    ...
    postings_dict: Dictionary mapping:
        ...
        Seperti namanya, "Dictionary" diimplementasikan sebagai python's Dictionary
          yang memetakan term ID (integer) ke 5-tuple:
           ...
           5. max_tf_in_postings_list : TF maksimum pada postings list term terkait.
    ...
    def __enter__(self):
        ...
        with open(self.metadata_file_path, 'rb') as f:
            ...
            if isinstance(metadata, dict):
                self.postings_dict = metadata["postings_dict"]
                self.terms = metadata["terms"]
                self.doc_length = metadata["doc_length"]
                self.avg_doc_length = metadata.get("avg_doc_length", 0.0)
                self.min_doc_length = metadata.get("min_doc_length", 0)
                self.index_format_version = metadata.get("index_format_version", 2)
            else:
                # Fallback untuk format lama; disarankan re-index ulang.
    ...
    def __exit__(self, exception_type, exception_value, traceback):
        ...
        with open(self.metadata_file_path, 'wb') as f:
            ...
            pickle.dump({
                ...
                "postings_dict": self.postings_dict,
                "terms": self.terms,
...
class InvertedIndexReader(InvertedIndex):
    ...
    def get_max_tf(self, term):
        """Mengembalikan TF maksimum dari postings list untuk term tertentu."""
